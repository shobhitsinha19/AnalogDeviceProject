...
3. Sort on every column. The example shows sorting on username.
...
5. Search the entire table using the search bar. The search is supported on all columns.
...
7. Script
A script is provided inside the backend directory which can take multiple github usernames and add them to mongodb.
